...
public class web {
    public static boolean getfile(String WebUrl, String filename, boolean IsTemporary) {
        try {
            ...
            Launcher.log.add("Malformed URL Exception raised on " + WebUrl);
        } catch (FileNotFoundException err) {
            out.println("Error on " + filename + ", file was not found!");
            Launcher.log.add("Error on " + filename + ", file was not found!");
        } catch (UnknownHostException err) {
            out.println("Server was not able to be contacted");
            Launcher.log.add("Server was not able to be contacted");
        } catch (IOException err) {
            out.println("Local file could not be written, the root file might not have write permissions");
            Launcher.log.add("Local file could not be written, the root file might not have write permissions");
        }
        return false;
    }
}
